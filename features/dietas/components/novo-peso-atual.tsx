...
import { NovoPesoAtualForm } from "./novo-peso-atual-form";

export const NovoPesoAtual = () => {
  const { fechar, abrir, estaAberto } = usarNovoPesoAtual();
    ...
    return (
      <Dialog open={estaAberto} onOpenChange={handleOpenChange}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>Meu peso</DialogTitle>
          </DialogHeader>
          <div className="mx-5">
            <NovoPesoAtualForm />
          </div>
    ...
    <Drawer open={estaAberto} onOpenChange={handleOpenChange}>
      <DrawerContent ref={drawerRef}>
        <DrawerHeader className="text-center flex items-center justify-center gap-2">
          <DrawerTitle>Meu peso</DrawerTitle>
        </DrawerHeader>
        <div className="mx-5">
          <NovoPesoAtualForm />
        </div>
